...
class FullyConnected():
  ...
  def __init__(self, input_dim, output_dim, activation, threshold=True):
    ...
    self.weights = np.random.uniform(low=-1, high=1, size=(output_dim, input_dim))
    self.weights *= np.sqrt(2/self.input_dim)
    self.threshold = np.zeros(output_dim) if threshold else None
  ...
  def __repr__(self):
    msg = "FC: ({}, {})".format(self.input_dim, self.output_dim)
    return msg
